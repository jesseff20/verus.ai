...
interface Field {
  ...
  validation?: any;
  options?: Array<string | { value: string; label: string }>; // Para select, radio, checkbox
  min_length?: number;
  max_length?: number;
  pattern?: string;
...
interface AddFieldDialogProps {
  ...
  initialField?: Field;
  isEditing?: boolean;
  existingFields?: Field[]; // Para verificar IDs duplicados
  blueprintId?: string; // ID do Blueprint para buscar agentes de seção
}

export function AddFieldDialog({ children, onAdd, initialField, isEditing = false, existingFields = [], blueprintId }: AddFieldDialogProps) {
  ...
  // Função para gerar ID baseado no label
  const generateIdFromLabel = (label: string, existingIds: string[] = []): string => {
    ...
    // Verificar duplicatas e adicionar sufixo numérico se necessário
    let finalId = baseId;
    let counter = 2;
  ...
  // Flag para saber se o usuário editou o ID manualmente
  const [idManuallySet, setIdManuallySet] = useState(false);

  // Auto-gerar ID quando o label mudar (só se não foi editado manualmente)
  useEffect(() => {
    if (field.label && !isEditing && !idManuallySet) {
      const existingIds = existingFields.map(f => f.id);
      const generatedId = generateIdFromLabel(field.label, existingIds);

      // Só atualizar se o ID realmente mudou
      if (generatedId !== field.id) {
        setField(prev => ({ ...prev, id: generatedId }));
      }
    }
  }, [field.label, isEditing, existingFields, idManuallySet]);
  ...
  return (
    <Dialog open={isEditing ? true : open} onOpenChange={isEditing ? () => {} : setOpen}>
      {!isEditing && <DialogTrigger asChild>{children}</DialogTrigger>}
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{isEditing ? 'Editar Campo' : 'Adicionar Campo ao Formulário'}</DialogTitle>
          <DialogDescription className="sr-only">Configurar e salvar campo no formulário</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
                      ...
                      <div className="space-y-2">
                        {sectionAgents.map((agent) => (
                          <div key={agent.id} className="flex items-start space-x-3 p-2 rounded-md hover:bg-purple-100 dark:hover:bg-purple-900/30 transition-colors">
                            ...
                            <div className="flex-1">
                              <label
                                htmlFor={`section-agent-${agent.id}`}
                                className="text-sm font-medium leading-none cursor-pointer"
                              >
          ...
          <div className="flex justify-end gap-2 pt-4 border-t">
            ...
            <Button onClick={handleAdd} disabled={!field.id || !field.label}>
