"""
STJSimulationService -- Simula julgamento colegiado do STJ.

Composicao: 5 Ministros por Turma (6 turmas), 10 por Secao (3 secoes).
  - 1a Secao (1a+2a Turma): Direito Publico
  - 2a Secao (3a+4a Turma): Direito Privado
  - 3a Secao (5a+6a Turma): Direito Penal
...
class STJSimulationService:
    """Simula um julgamento colegiado do STJ com votos individuais dos ministros."""
    ...
    def _load_ministers(self) -> List[MinisterProfile]:
        """Carrega ministros do STJ com base na composicao configurada."""
        composition = self.config.get('composition', '1a_turma')
        qs = MinisterProfile.objects.filter(court_type='STJ', is_active=True)
        ...
        turma_map = {
            ...
            '5a_turma': '5a Turma',
            '6a_turma': '6a Turma',
            '1a_secao': ['1a Turma', '2a Turma'],
            '2a_secao': ['3a Turma', '4a Turma'],
            '3a_secao': ['5a Turma', '6a Turma'],
        }

        filter_val = turma_map.get(composition)
        if isinstance(filter_val, list):
            qs = qs.filter(turma__in=filter_val)
        elif filter_val:
            qs = qs.filter(turma=filter_val)

        ministers = list(qs)

        # If no ministers in DB, create generic profiles
        if not ministers:
    ...
    def _create_generic_profiles(self, composition: str) -> List[MinisterProfile]:
        """Cria perfis genericos de ministros do STJ."""
        count = 10 if 'secao' in composition else 5
        area = 'Direito Publico'
        turma = '1a Turma'

        if '2a_secao' in composition or '3a_turma' in composition or '4a_turma' in composition:
            area = 'Direito Privado'
            turma = '3a Turma'
        elif '3a_secao' in composition or '5a_turma' in composition or '6a_turma' in composition:
            area = 'Direito Penal'
            turma = '5a Turma'

        generic_names = [
            ('Min. Fernando Augusto', 'centrista', 'Tecnico e objetivo'),
            ('Min. Patricia Mendes', 'progressista', 'Didatico e fundamentado'),
            ('Min. Ricardo Lopes', 'conservador', 'Formal e processualista'),
            ('Min. Helena Costa', 'pragmatico', 'Pragmatico e conciso'),
            ('Min. Marcos Andrade', 'centrista', 'Equilibrado e doutrinario'),
            ('Min. Claudia Ferreira', 'progressista', 'Garantista e detalhado'),
            ('Min. Jose Almeida', 'conservador', 'Legalista estrito'),
            ('Min. Ana Beatriz Silva', 'pragmatico', 'Consequencialista'),
            ('Min. Paulo Henrique', 'centrista', 'Moderado e tecnico'),
            ('Min. Lucia Rodrigues', 'progressista', 'Humanista e contextual'),
        ]
        ...
        for i in range(min(count, len(generic_names))):
            ...
            profile = MinisterProfile(
                court_type='STJ',
                name=name,
                turma=turma if i < 5 else turma.replace('a', 'a'),
                judicial_philosophy=philosophy,
                specialty_areas=[area, 'Direito Processual'],
                notable_positions=[],
                profile_data={
                    'writing_style': style,
                    'tendencies': [],
                    'key_framework': 'Legislacao infraconstitucional',
                },
            )
    ...
    def stream_simulation(self) -> Generator[Dict, None, None]:
        """Generator que produz eventos SSE para a simulacao do STJ."""
        ...
        try:
            ...
            yield self._event('complete', 'Simulacao do STJ concluida.')

        except Exception as e:
            logger.exception(f'[stj_simulation] Erro na simulacao {self.simulation.id}: {e}')
    ...
    def _phase_relatorio(self, case_context: str, relator: MinisterProfile) -> Generator[Dict, None, str]:
        ...
        action_type = self.config.get('action_type', 'REsp')

        prompt = (
            f"Voce e o Ministro {relator.name} do Superior Tribunal de Justica (STJ).\n"
            f"Tipo de recurso: {action_type}\n\n"
            f"Elabore o RELATORIO do caso a seguir, no formato tradicional do STJ:\n"
            f"1. Identificacao das partes e do recurso\n"
            f"2. Sintese do acordao recorrido\n"
            f"3. Razoes do recurso especial\n"
            f"4. Contrarrazoes\n"
            f"5. Parecer do Ministerio Publico Federal (se aplicavel)\n"
            f"6. Admissibilidade do recurso\n\n"
            f"CASO:\n{case_context}\n\n"
            f"Seja objetivo e tecnico, no estilo dos relatorios do STJ. "
            f"Lembre-se: o STJ analisa violacao a legislacao INFRACONSTITUCIONAL (nao constitucional)."
        )

        full_text = ''
        for chunk_text in self._stream_llm(prompt, 'Voce e um ministro do STJ elaborando relatorio.'):
    ...
    def _phase_vote(
        ...
        """Gera o voto de um ministro do STJ."""
        ...
        prompt = (
            f"Voce e o Ministro {minister.name} do Superior Tribunal de Justica (STJ).\n"
            ...
            f"PROFIRA SEU VOTO no estilo do STJ:\n"
            f"1. Fundamente com base na LEGISLACAO INFRACONSTITUCIONAL (Codigo Civil, CPC, CDC, etc.)\n"
            f"2. Cite jurisprudencia do STJ (sumulas, recursos repetitivos)\n"
            f"3. Analise se houve violacao a lei federal\n"
            f"4. Ao final, DECLARE EXPRESSAMENTE se vota pelo PROVIMENTO ou DESPROVIMENTO "
            f"do recurso especial\n"
            f"5. Se divergir do relator, justifique a divergencia\n\n"
            f"IMPORTANTE: O STJ NAO analisa materia constitucional (isso e competencia do STF). "
            f"Foque na legislacao infraconstitucional.\n\n"
        ...
        for chunk_text in self._stream_llm(
            prompt,
            f'Voce e o Min. {minister.name} do STJ proferindo voto. '
    ...
    def _phase_proclamacao(self, votes: List[Dict], case_context: str) -> Generator[Dict, None, str]:
        ...
        prompt = (
            f"Voce e o Presidente da Turma do STJ proclamando o resultado de um julgamento.\n\n"
            f"VOTOS:\n{votes_summary}\n\n"
            f"PLACAR: {provimento} x {desprovimento}\n\n"
            f"Proclame o resultado no formato oficial do STJ:\n"
            f"1. Resumo do placar\n"
            f"2. Resultado (provido/desprovido, conhecido/nao conhecido)\n"
            f"3. Quem votou com o relator e quem divergiu\n"
            f"4. Determinacoes decorrentes\n\n"
            f"Use o formato oficial: 'A Turma, por maioria/unanimidade...'"
        )

        full_text = ''
        for chunk_text in self._stream_llm(prompt, 'Voce e o Presidente da Turma do STJ proclamando resultado.'):
            full_text += chunk_text
    ...
    def _phase_strategic_report(
        ...
        prompt = (
            ...
            f"Analise o resultado do julgamento no STJ:\n\n"
            f"CASO:\n{case_context}\n\n"
            f"PLACAR:\n{votes_summary}\n\n"
            f"PROCLAMACAO:\n{proclamacao}\n\n"
            ...
            f"3. Possibilidade de embargos de divergencia\n"
            f"4. Viabilidade de Recurso Extraordinario ao STF\n"
            f"5. Impacto da decisao (sumula, recurso repetitivo)\n"
        ...
        for chunk_text in self._stream_llm(
            prompt,
            'Voce e um consultor juridico estrategico especializado em STJ.',
        ):
            full_text += chunk_text
