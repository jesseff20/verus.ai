...
def flow_summary(organ) -> dict:
    ...
    """
    qs = FlowInstance.objects.filter(organ=organ)
    total = qs.count()
    by_status = {
...
def flows_by_template(organ, days: int = 30) -> list[dict]:
    ...
    qs = (
        ...
        .filter(organ=organ, created_at__gte=since)
...
def avg_completion_time_by_template(organ) -> list[dict]:
    ...
    qs = (
        FlowInstance.objects
        .filter(organ=organ, status='completed',
...
def tasks_by_role(organ, days: int = 30) -> list[dict]:
    ...
    qs = (
        ...
        .filter(instance__organ=organ, created_at__gte=since)
